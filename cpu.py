...
def run_generation(job_id, prompt, duration, steps, cfg_scale, model_key):
    try:
        ...
        jobs[job_id]["device"] = "cpu"
        jobs[job_id]["status"] = "loading_model"

        model, model_config = get_model(model_key)
...
def generate():
    ...
    jobs[job_id] = {"status": "queued", "filename": None, "error": None, "model_key": model_key, "device": "cpu"}
...
if __name__ == "__main__":
    if not HF_TOKEN:
        ...
        exit(1)
    print(f"\n CPU mode — open http://localhost:5001\n")
